...
VERSION=v${1:-$(curl https://iojs.org/download/nightly/index.json | sed -e 's/^.*"version":"\([^"]*\)".*$/\1/' | head -n 2 | tail -n -1 | cut -c 2-)}
IOJS=iojs-${VERSION}-linux-x64

echo "> Downloading $VERSION of iojs"
curl -s https://iojs.org/download/nightly/${VERSION}/${IOJS}.tar.xz | tar xvfJ -
